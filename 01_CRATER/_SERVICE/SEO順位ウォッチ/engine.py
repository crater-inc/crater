...
やること:
  1. 対象サイトの順位をSearch Consoleから取得して順位履歴.jsonに追記
  2. 観察期間が明けた改善の効果を実測で判定する
  3. 次に改善すべき候補を洗い出す
  4. Claudeに渡すサマリー（summary.txt）を書き出す
...
def 最新の結果(サイト):
    履歴 = 読む(順位履歴ファイル, {"測定": []})
    該当 = [m for m in 履歴["測定"] if m["サイト"] == サイト]
...
def main():
    ...
    候補群 = {s: 候補を洗い出す(s, ログ) for s in 行く先}
    変動群 = {s: 変動を出す(s) for s in 行く先}

    print(サマリーを書く(判定結果, 候補群, 変動群))
