...
やること:
  ...
  3. 「改善」「提案のみ」のサイトについて、次に改善すべき候補を洗い出す
...
import wp   # 同じフォルダの wp.py（WordPressの認証情報があるかの確認に使う）
...
# サイトごとの扱い
#   改善     … 測定して、候補を選び、タイトル等を自動で直す（週の上限まで）
#   提案のみ … 候補を選んで改善案を報告（LINE）に書くだけ。記事には触らず、承認後に反映する
#   測定のみ … 順位を記録して異常を見るだけ。記事には触らない
#   ※「改善」でもWordPressの認証情報が見つからない週は、自動で「鍵待ち」（測定のみ）になる
# 週の上限 … 1週間に直してよい記事の最大数（提案のみのサイトは提案する件数）。事業につながる候補がなければ少なくてよい
# 最低表示・最大順位 … 候補に入れる条件。記事が少ないサイトは表示回数も少ないので緩めにしている
# 対象URL … 候補にするページのURLの条件（正規表現）。トップページはどのサイトでも候補にしない
対象サイト = {
    "chics.top":      {"扱い": "改善",     "週の上限": 5, "最低表示": 20, "最大順位": 20},
    "years.design":   {"扱い": "改善",     "週の上限": 5, "最低表示": 5,  "最大順位": 30,
                       "対象URL": r"^https://years\.design/info/\d+"},   # ブログ記事だけ
    # 実績のタイトル＝一覧に出る作品名なので、自動では変えない。お知らせ（/info/）も対象外
    "crater.co.jp":   {"扱い": "提案のみ", "週の上限": 2, "最低表示": 5,  "最大順位": 30,
                       "対象URL": r"^https://crater\.co\.jp/works/[^/]+/[^/]+/"},
    "apollos.jp":     {"扱い": "改善",     "週の上限": 2, "最低表示": 5,  "最大順位": 30,
                       # 固定ページ（会社概要・サービス等）は除く
                       "対象URL": r"^https://apollos\.jp/(?!(contact|company|about|service|philosophy|info|sample-page)/)"},
    "birth.business": {"扱い": "測定のみ", "週の上限": 2, "最低表示": 5,  "最大順位": 30},  # WordPressではない・まだ検索データなし
    "acurry.jp":      {"扱い": "測定のみ", "週の上限": 2, "最低表示": 5,  "最大順位": 30},  # Shopify。書き込む手段が未整備
}
...
# 指名検索（社名・ブランド名そのものの検索）。もともと上位にいて、タイトルを直す意味がないので候補から外す
指名語 = {
    "chics.top": r"chics|シックス",
    "years.design": r"^years|イヤーズ",
    "crater.co.jp": r"クレーター|crater",
    "apollos.jp": r"apollos|アポロス",
    "birth.business": r"^birth\b|birth\.business",
    "acurry.jp": r"a\s*curry|acurry|accurry|アカリー|akari\s*curry",
}
...
def 今週の扱い(サイト):
    """設定上の扱いに、WordPressの認証情報の有無を反映する。認証情報が無い週は測定のみ。"""
    扱い = 対象サイト[サイト]["扱い"]
    if 扱い == "改善" and (サイト not in wp.サイト設定 or not wp.認証情報を探す(サイト)):
        return "鍵待ち"
    return 扱い
...
def 候補を洗い出す(サイト, ログ):
    ...
    どれを選ぶかはClaudeが判断するので、週の上限より多めに（上限の4倍・最低12件）渡す。
    """
    設定 = 対象サイト[サイト]
    上限 = max(12, 設定["週の上限"] * 4)
    ...
    for r in 最新の結果(サイト):
        ...
        if (サイト, r["ページ"]) in 観察中ページ:
            continue
        # トップページは自動では変えない。対象URLが決まっているサイトは、それ以外のページも外す
        if urllib.parse.urlparse(r["ページ"]).path in ("", "/"):
            continue
        if 設定.get("対象URL") and not re.search(設定["対象URL"], r["ページ"]):
            continue
        if スパムか(r["キーワード"]):
            continue
        if 指名語.get(サイト) and re.search(指名語[サイト], r["キーワード"], re.I):
            continue
        if r["表示回数"] < 設定["最低表示"] or r["順位"] > 設定["最大順位"]:
            continue
        差 = 期待との差(r)
...
def サマリーを書く(扱い, 判定結果, 候補群, 変動群, 異常群):
    改善サイト = [f"{s}（今週最大{c['週の上限']}本）" for s, c in 対象サイト.items() if 扱い[s] == "改善"]
    提案サイト = [f"{s}（最大{c['週の上限']}件）" for s, c in 対象サイト.items() if 扱い[s] == "提案のみ"]
    鍵待ち = [s for s in 対象サイト if 扱い[s] == "鍵待ち"]
    行 = [f"■ SEO順位ウォッチ {今日().isoformat()}", "",
          f"【改善してよいサイトと今週の上限】{' / '.join(改善サイト) or 'なし'}",
          "（上限は最大値。事業につながる候補がなければ少なくてよい。ここに無いサイトの記事には絶対に触らないこと）"]
    if 提案サイト:
        行.append(f"【提案だけするサイト（記事には触らず、改善案を報告に書くだけ）】{' / '.join(提案サイト)}")
    if 鍵待ち:
        行.append(f"【WordPressの認証情報が未登録（今週は測定のみ）】{', '.join(鍵待ち)}")
    行.append("")
    ...
    for サイト, 候補 in 候補群.items():
        上限 = 対象サイト[サイト]["週の上限"]
        if 扱い[サイト] == "提案のみ":
            行.append(f"【{サイト} 提案する候補（惜しい順・最大{上限}件。記事には触らない）】")
        else:
            行.append(f"【{サイト} 次に改善する候補（惜しい順・今週最大{上限}本）】")
        if not 候補:
...
def main():
    ...
    扱い = {s: 今週の扱い(s) for s in 対象サイト}
    候補群 = {s: 候補を洗い出す(s, ログ) for s in 行く先 if 扱い[s] in ("改善", "提案のみ")}
    変動群 = {s: 変動を出す(s) for s in 行く先}
    異常群 = {s: 異常を探す(s) for s in 行く先}

    print(サマリーを書く(扱い, 判定結果, 候補群, 変動群, 異常群))
