...
#   認証待ち … 改善する予定だが、WordPressのアプリケーションパスワードが未登録。届くまでは測定のみ
#   測定のみ … 順位を記録して異常を見るだけ。記事には触らない
# 週の上限 … 1週間に直してよい記事の最大数。事業につながる候補がなければ少なくてよい
# 最低表示・最大順位 … 候補に入れる条件。記事が少ないサイトは表示回数も少ないので緩めにしている
対象サイト = {
    "chics.top":      {"扱い": "改善",     "週の上限": 5, "最低表示": 20, "最大順位": 20},
    "years.design":   {"扱い": "認証待ち", "週の上限": 5, "最低表示": 5,  "最大順位": 30},
    "crater.co.jp":   {"扱い": "認証待ち", "週の上限": 2, "最低表示": 5,  "最大順位": 30},
    "apollos.jp":     {"扱い": "認証待ち", "週の上限": 2, "最低表示": 5,  "最大順位": 30},
...
def サマリーを書く(判定結果, 候補群, 変動群, 異常群):
    改善サイト = [f"{s}（今週最大{c['週の上限']}本）" for s, c in 対象サイト.items() if c["扱い"] == "改善"]
    認証待ち = [s for s, c in 対象サイト.items() if c["扱い"] == "認証待ち"]
    行 = [f"■ SEO順位ウォッチ {今日().isoformat()}", "",
          f"【改善してよいサイトと今週の上限】{' / '.join(改善サイト)}",
          "（上限は最大値。事業につながる候補がなければ少なくてよい。ここに無いサイトの記事には絶対に触らないこと）"]
    if 認証待ち:
        行.append(f"【認証待ち（今週は測定のみ）】{', '.join(認証待ち)}")
    行.append("")
    ...
    for サイト, 候補 in 候補群.items():
        行.append(f"【{サイト} 次に改善する候補（惜しい順・今週最大{対象サイト[サイト]['週の上限']}本）】")
        if not 候補:
...
def main():
    ...
    候補群 = {s: 候補を洗い出す(s, ログ) for s in 行く先 if 対象サイト[s]["扱い"] == "改善"}
